...
class OutputClass:
    ...
    def __init__(self, code, name, semestersOffered, credits, notes):
        self.name, self.code, self.semestersOffered, self.credits, self.notes = name, code, semestersOffered, int(
            credits), notes
...
class OutputWriter:
    ...
    def write(self):
        ...
        for y in range(len(self.years)):
            ...
            worksheet.conditional_format(f'A{(y * 9) + 3}:A{(y * 9) + 3}',
                                         {'type': 'no_errors', 'format': semester_border_top_left})
            worksheet.conditional_format(f'B{(y * 9) + 3}:B{(y * 9) + 3}',
                                         {'type': 'no_errors', 'format': semester_border_top_right})
            worksheet.conditional_format(f'A{(y * 9) + 4}:A{(y * 9) + 10}',
                                         {'type': 'no_errors', 'format': semester_border_left})
            worksheet.conditional_format(f'B{(y * 9) + 4}:B{(y * 9) + 10}',
                                         {'type': 'no_errors', 'format': semester_border_right})
            worksheet.conditional_format(f'A{(y * 9) + 11}:A{(y * 9) + 11}',
                                         {'type': 'no_errors', 'format': semester_border_bottom_left})
            worksheet.conditional_format(f'B{(y * 9) + 11}:B{(y * 9) + 11}',
                                         {'type': 'no_errors', 'format': semester_border_bottom_right})
            worksheet.conditional_format(f'C{(y * 9) + 3}:C{(y * 9) + 3}',
                                         {'type': 'no_errors', 'format': semester_border_top_left})
            worksheet.conditional_format(f'D{(y * 9) + 3}:D{(y * 9) + 3}',
                                         {'type': 'no_errors', 'format': semester_border_top_right})
            worksheet.conditional_format(f'C{(y * 9) + 4}:C{(y * 9) + 10}',
                                         {'type': 'no_errors', 'format': semester_border_left})
            worksheet.conditional_format(f'D{(y * 9) + 4}:D{(y * 9) + 10}',
                                         {'type': 'no_errors', 'format': semester_border_right})
            worksheet.conditional_format(f'C{(y * 9) + 11}:C{(y * 9) + 11}',
                                         {'type': 'no_errors', 'format': semester_border_bottom_left})
